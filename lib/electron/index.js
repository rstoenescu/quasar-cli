...
class ElectronRunner {
  ...
  build (quasarConfig) {
    const
      ...
      useElectronBuilder = cfg.electron.useElectronBuilder || false,
      builderConfig = cfg.electron.builder,
    ...
    return new Promise((resolve, reject) => {
      ...
      return new Promise((resolve, reject) => {
        log(`Packaging app with electron-${useElectronBuilder ? 'builder' : 'packager'}...`)
        log()

        if (useElectronBuilder) {
          const builder = require(appPaths.resolve.app('node_modules/electron-builder'))
          builder.build({ config: builderConfig })
            .then(() => {
              log()
              log(`[SUCCESS] electron-builder built the app`)
              log()
              resolve()
            })
            .catch(err => {
              log()
              warn(`[FAIL] electron-builder could not build`)
              console.log(err + '\n')
              reject()
            })
        } else {
