...
module.exports = function (argv, cmd) {
  if (cmd === 'mode') {
    if (![undefined, 'pwa', 'cordova', 'electron', 'ssr', 'firebase'].includes(argv.add)) {
      warn(`⚠️  Unknown mode "${ argv.add }" to add`)
      warn()
      process.exit(1)
    }
    if (![undefined, 'pwa', 'cordova', 'electron', 'ssr', 'firebase'].includes(argv.remove)) {
  ...
  if (!['spa', 'pwa', 'cordova', 'electron', 'ssr', 'firebase'].includes(argv.mode)) {
