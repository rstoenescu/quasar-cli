...
module.exports = function (argv, cmd) {
  if (cmd === 'mode') {
    if (![undefined, 'pwa', 'cordova', 'electron', 'unit_test'].includes(argv.add)) {
      warn(`Unknown mode "${ argv.add }" to add`)
      warn()
      process.exit(1)
    }
    if (![undefined, 'pwa', 'cordova', 'electron', 'unit_test'].includes(argv.remove)) {
  ...
  if (!['spa', 'pwa', 'cordova', 'electron', 'unit-test'].includes(argv.mode)) {
  ...
  if (cmd === 'build' && argv.mode === 'unit_test') {
